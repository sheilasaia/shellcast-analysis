...
sga_bounds_raw <- st_read(paste0(sga_spatial_data_input_path, "sga_bounds_raw_valid_wgs84.shp"))
...
sga_bounds <- sga_bounds_raw %>%
  dplyr::mutate(grow_area_class = if_else(HA_CLASSID == "APP", "approved",
  ...
  dplyr::arrange(grow_area, grow_area_class, ha_name) %>%
...
# export
# st_write(sga_bounds_albers, paste0(sga_spatial_data_output_path, "sga_bounds_tidy_albers.shp"))
# st_write(sga_bounds_wgs84, paste0(sga_spatial_data_output_path, "sga_bounds_tidy_wgs84.shp"))
...
# export data
st_write(sga_bounds_simple_wgs84, paste0(sga_spatial_data_output_path, "sga_bounds_simple_wgs84.shp"), delete_layer = TRUE)
...
st_write(sga_bounds_buffer_wgs84, paste0(sga_spatial_data_output_path, "sga_bounds_10kmbuf_wgs84.shp"), delete_layer = TRUE)


# ---- 8. tidy sga boundaries by sga and class ----
# summarize boundaries (equivalent to spatial dissolve)
sga_bounds_class_albers <- sga_bounds_albers %>%
  dplyr::group_by(grow_area, grow_area_class, ha_name_fix) %>% # group by sga name and class
  dplyr::summarize() %>%
...
# export data
# st_write(sga_bounds_class_wgs84, paste0(sga_spatial_data_output_path, "sga_bounds_class_wgs84.shp"), delete_layer = TRUE)
