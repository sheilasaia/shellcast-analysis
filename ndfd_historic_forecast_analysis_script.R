...
ndfd_hist_pop12_data_albers_shp <- st_as_sf(ndfd_hist_pop12_data, coords = c("longitude", "latitude"), crs = ndfd_proj4, dim = "XY") #%>%
...
# ----- plot spatial data ----
# plot state bounds
pdf(file = paste0(figure_path, "test_2.pdf"), width = 11, height = 8.5)
ggplot() +
  #geom_sf(data = midatlantic_bounds_shp) #+
  geom_sf(data = ndfd_pop12_data_bbox_shp, fill = NA, color = "red")
dev.off()
